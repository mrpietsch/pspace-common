 ...
 */
public class ObjectWithIdEditor<T extends ObjectWithID> extends PropertyEditorSupport implements Formatter<T> {

    /* fields */

    // protected final static Log log = LogFactory.getLog(ObjectWithIdEditor.class);

    private final GenericManager<T> manager;

    public ObjectWithIdEditor(GenericManager<T> manager) {
        assert manager != null;
